...
const Navbar = () => {
    ...
    // Active Menu
    const getActiveLinkColor = (isActive) => `${isActive ? "text-yellowV" : "text-whiteV"} text-lg font-fontTitle hover:text-yellowV transform hover:rotate-[-2deg] hover:scale-125 transition duration-300 py-2 sm:py-0`;
    ...
    return (
        <nav className='fixed top-0 w-full p-2 flex justify-between items-center bg-goldVt z-50 border-b-2 border-whiteVt'>
            <div className={`${showMenu ? "flex" : "hidden"} flex-col items-center w-full absolute top-full pb-5 sm:flex sm:relative sm:flex-row sm:pb-0 sm:justify-around`}>
